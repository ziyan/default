...
GLWidget::GLWidget(World* world, Robot *robot, QWidget *parent)
	...
	for(int i = 0; i < 361; i++)
		laser_angles[i] = (double)(i - 180) / 360.0 * M_PI;
...
void GLWidget::paintGL() {
	...
	glLoadIdentity();
	gluPerspective(60.0, (GLdouble)width/(GLdouble)height, 0.001, 400.0);
...
void GLWidget::drawRobot(double x, double y, double yaw, double ranges[361]) {
	if(eye_r < 1.5) return;
	glPushMatrix();
...
void GLWidget::drawGrid() {
	...
	for(double x = -GRID_X_MAX; x <= GRID_X_MAX; x += 1.0) {
		glVertex3f(x, -GRID_Y_MAX, -0.1);
		glVertex3f(x, GRID_Y_MAX, -0.1);
	}
	for(double y = -GRID_Y_MAX; y <= GRID_Y_MAX; y += 1.0) {
...
void GLWidget::drawMap() {
	...
	uint8_t *texture = map.getTexture();
	
	if(texture) {
		glTexImage3D(GL_TEXTURE_3D, 0, 3, map.getWidth(), map.getHeight(), map.getDepth(), 0, GL_RGB, GL_UNSIGNED_BYTE, texture);
